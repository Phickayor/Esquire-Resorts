...
function AddReservation(props) {
    ...
    const HandleSubmit = ((e) => {
        ...
        const sdepatureDate = departureDateContainer.current.value;
        ...
        fetch("http://localhost:8080/booking", {
            ...
            body: JSON.stringify({ sname, sroomname, sarrivalDate, sdepatureDate, sguestNumber, sprice, sref })
        }).then((res) => {
            return res.json()
        }).then((data) => {
    ...
    return (
        <form className='flex flex-col' onSubmit={HandleSubmit}>
            <fieldset className='border-2 lg:space-y-5 lg:grid grid-cols-2 lg:gap-x-10 rounded-xl px-5'>
                <legend className='text-xl px-5 text-center md:text-2xl font-bold'>Add Reservation</legend>
                <div>
            ...
            <button type="submit" className="bg-slate-700 self-center font-semibold text-white px-10 py-3 mt-5 rounded-xl">
