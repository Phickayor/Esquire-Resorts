...
function Details() {
    ...
    const [guestFilterSelected, setGuestFilterSelected] = useState("Guest Number")
    const guestFilters = useRef("")
    const roomFilters = useRef("")
    const spinner = <FontAwesomeIcon icon={faSpinner} className='self-center fa-spin text-4xl lg:text-6xl' />
                    ...
                    (content == "details") ?
                        (<div className="mx-auto md:w-10/12 w-11/12 text-slate-700 my-10 flex flex-col">
                            <button
                                ...
                                onClick={changeContent}>ADD USER {plus}</button>
                            <h1 className='text-2xl md:text-3xl font-mono font-semibold text-center'>Search for Reservation</h1>
                            <input type="search" placeholder="Input reference no..."
                                required
                                autoComplete="true"
                                name="reference"
                                value={ref}
                            ...
                            <div className='block mx-auto w-8/12 md:flex justify-between gap-5 space-y-5 md:space-y-0'>
                                <div className='w-full'>
                                    ...
                                    <div className="z-10 my-2  rounded-lg shadow bg-gray-700">
                                        <ul ref={roomFilters} className="py-2 hidden text-sm text-gray-700 dark:text-gray-200">
                                            <li onClick={setRoomsFilter}
                                                className="block px-4 py-2 hover:bg-gray-600 hover:text-white">Deluxe King
                                            </li>
                                            <li onClick={setRoomsFilter}
                                                className="block px-4 py-2 hover:bg-gray-600 hover:text-white">Cozy Exeutive
                                            </li>
                                            <li onClick={setRoomsFilter}
                                                className="block px-4 py-2 hover:bg-gray-600 hover:text-white">Deluxe King Extra
                                            </li>
                                            <li onClick={setRoomsFilter}
                                                className="block px-4 py-2 hover:bg-gray-600 hover:text-white">Diamond Suite
                                            </li>
                                            <li onClick={setRoomsFilter}
                                                className="block px-4 py-2 hover:bg-gray-600 hover:text-white">Studio Room
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                                ...
                                <div className='w-full'>
                                    <button onClick={() => ToggleFilter("guests")}
                                        className="justify-between text-white font-medium rounded-lg text-md p-4 text-center inline-flex items-center bg-blue-600 hover:bg-blue-700 focus:ring-blue-800 w-full" type="button">
                                        {guestFilterSelected}{down}
                                    </button>
                                    <div ref={guestFilters} className="z-10 my-2 hidden rounded-lg shadow bg-gray-700">
                                        <ul className="py-2 text-sm text-gray-700 dark:text-gray-200">
                                            <li onClick={setGuestFilter}
                                                className="block px-4 py-2 hover:bg-gray-600 hover:text-white">Just One
                                            </li>
                                            <li onClick={setGuestFilter}
                                                className="block px-4 py-2 hover:bg-gray-600 hover:text-white">2 Guests
                                            </li>
                                            <li onClick={setGuestFilter}
                                                className="block px-4 py-2 hover:bg-gray-600 hover:text-white">3 Guests
                                            </li>
                                            <li onClick={setGuestFilter}
                                                className="block px-4 py-2 hover:bg-gray-600 hover:text-white">4 Guests
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                            ...
                            {reservations ? (
                                <div className='grid md:grid-cols-2 lg:grid-cols-3 md:py-10 py-6 gap-6 px-5 text-center'>
                                    ...
                                    {reservations.map((reservation, index) => {
                                        return (
                                            <div key={index} className='hover:bg-purple-200 cursor-pointer ease-in-out duration-300 3py-5 w-full border-4 border-slate-500 rounded-3xl'>
                                                <div className='space-y-8 my-4'>
                                                    <h3 className={`text-3xl font-mono font-semibold ${poppins.className}`}>{reservation.rname}</h3>
                                                    <p className='text-2xl text-slate-600'></p>
                                                    <ul className='list-style-none space-y-4'>
                                                        <li><strong>Name:</strong> {reservation.name}</li>
                                                        <li><strong>Arrival Date:</strong> {reservation.arrivalDate}</li>
                                                        <li><strong>Departure Date:</strong> {reservation.depatureDate}</li>
                                                        <li><strong>Guest Number:</strong> {reservation.guestNumber}</li>
                                                        <li><strong>Price:</strong> ₦ {reservation.price}</li>
                                                        <li><strong>Reference:</strong> {reservation.ref}</li>
                                                    </ul>
                                                </div>
                                            </div>
                                        )
                        ...
                        (<div className="mx-auto md:w-10/12 w-11/12 text-slate-700 my-10 flex flex-col">
                            <button
                                className='justify-between mb-5 text-white font-semibold rounded-lg text-md p-4 text-center items-center self-end bg-blue-600 hover:bg-blue-700'
                                onClick={changeValue}>View Reservations</button>
